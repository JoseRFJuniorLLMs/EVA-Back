...
async def list_idosos(
    ...
    if user_role in ['admin', 'profissional']:
        query = text("""
            ...
            FROM idosos i
            WHERE i.ativo = true
            AND (CAST(:nome AS TEXT) IS NULL OR LOWER(i.nome) LIKE LOWER(:nome_pattern))
            AND (CAST(:cpf AS TEXT) IS NULL OR i.cpf = :cpf)
            AND (CAST(:telefone AS TEXT) IS NULL OR i.telefone = :telefone)
            ORDER BY i.id DESC
