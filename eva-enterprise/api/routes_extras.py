...
async def get_idoso_report(id: int, db: Session = Depends(get_db)):
    ...
    idoso = idoso_repo.get_by_id(id)
    if not idoso:
        raise HTTPException(status_code=404, detail="Idoso not found")

    sinais = sinais_repo.get_weekly_report(id)
    insights = alerta_repo.get_insights(id)
    
    # Need to wait for async method
...
# Real implementation in webhooks.py via WebSocket, but maybe user wants an HTTP entry point?
@router.post("/gemini/session")
def create_session_instruction():
